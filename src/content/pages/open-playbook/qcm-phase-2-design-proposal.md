...
| 7a | **External CRM pattern study (Twenty)** ... `twentyhq/twenty` open-source CRM (47k stars; v2.8.0 May 2026; React + NestJS + raw Postgres + Nx monorepo; AGPL v3 + Twenty.com Commercial License dual-license for `/* @license Enterprise */` files) evaluated 2026-05-27 per D-061 as potential acceleration / replacement / partial absorption for QCM; verdict locked = **NOT adopted as base** on 5 load-bearing grounds (AGPL+Enterprise vendor-lock recreates SuiteDash trap per D-003; stack fragmentation vs QWF app family Supabase+SvelteKit+CF Pages convergence; state-centric data model per D-017–D-022 cannot be represented natively on Twenty's "object with fields" primitive; fails 1000% (i) bar via documented gaps in custom auth + multi-tenancy + RLS; forward-cost asymmetry with 28 migrations + 30 edge functions + 60 ratified decisions live). Twenty IS valuable as a study target for Phase 3+ pattern inspiration: kanban / board view patterns → HQ state-pipeline UX; workflow automation graph model → deferred `workflows` + `workflow_runs` tables (§16.4 item 36); AI agent skill definitions → QCM agentic surface design; `defineObject` / `defineField` schema-as-TS ergonomics → custom-field-definition API when one is needed; permission model patterns → program-partition + RLS evolution | Full adoption rejected per D-061 senior-dev / long-term / bulletproof framing; pattern-study scope = extract ideas into QCM-shaped recommendations without code adoption, partial absorption, or sync layer | When **Phase 2.5 portal ships** AND **Phase 3 multi-perspective view layer design surfaces**, spawn a 1-2 day Twenty-source-code study sub-agent task. Output: single research note at `002 Projects/_Quietly CRM/Brainstorm Prompts/Twenty-Pattern-Study.md`. **NO code adoption. NO partial absorption. NO sync layer. Research note only.** Re-litigating the adoption verdict requires honest-deviation reporting per `[[feedback_kickoff_claims_must_match_deployed_reality]]` (D-061 is durable). |
...
| 15 | **`contacts.additional_methods` JSONB column drop** | Phase 2.0 backfills into contact_methods; column kept through 2.0+2.1 as fallback | ✅ DONE 2026-05-26 Phase 2.2.1 Migration 028 (atomic with `qcm_contact_append_method` refactor to write into `contact_methods`); D-059 ratified narrow scope |
| 16 | **`contacts.email` / `phone` / `linkedin_url` legacy single-column drop** | Phase 2.0+2.1 retains as fallback; Phase 2.2 deprecates; Phase 3.x drops. Phase 2.2.1 deferral 2026-05-26 documented ~4 RPCs (`qcm_merge_candidates`, `qcm_contact_merge`, `qcm_contact_patch`, `qcm_contact_append_method`) + 1 edge function (`quietly-crm/supabase/functions/contacts/index.ts` ... `CONTACT_FIELDS`, `PATCH_ALLOWED_FIELDS`, `PATCH_NULLABLE_FIELDS`, `PATCH_WRITETHROUGH_FIELDS`) actively dependent on legacy columns. Additional prerequisite per D-059: 14-orphan-contact class (contacts with NO `contact_states` rows) must be remediated BEFORE item 16 ships; sub-agent's Aretha Simons example is one instance of the broader class | **Phase 3 Migration** after (a) all callers migrated to `contact_methods` reads + (b) 14-orphan-contact class triaged (synthesize personal state for genuine contacts; hard-delete genuinely abandoned rows). Pre-Phase-3 audit query: `SELECT count(*) FROM contacts c WHERE c.deleted_at IS NULL AND NOT EXISTS (SELECT 1 FROM contact_states cs WHERE cs.contact_id=c.id AND cs.deleted_at IS NULL);` (must return 0 before item 16 ships) |
| 17 | **`contact_employment_history` table drop** (it becomes a VIEW filtering contact_states WHERE state_type='employment' at Phase 2.0+2.1; the table itself drops later) | Phase 2.0+2.1 keeps the table as legacy compatibility. Phase 2.2.1 deferral 2026-05-26 documented 5 RPCs (`fn_emit_event`, `qcm_contact_export_bundle`, `qcm_contact_merge`, `qcm_organization_patch`, `qcm_purge_test_fixtures`) + data integrity blockers: 6 CEH rows carry `role`/`notes` columns not in `contact_states`; 67 of 73 employment states have no matching CEH row (CEH is subset; not 1:1 source) | **Phase 2.2.2 or Phase 3.0** when (a) 5 RPCs refactored to read from `contact_states` filtered to `state_type='employment'`, (b) 6 CEH-only `role`/`notes` data migrated into `contact_states.role_title` / new `contact_states.notes` (or absorbed into `state_label`), (c) 67 unsynced employment states audited for CEH-shape coverage |
| 15a (new) | **14-orphan-contact class remediation** (contacts with no `contact_states` rows) | Per Phase 2.2.1 Q2 verdict + D-059 sub-agent Closing Flag #5: 14 contacts pre-date personal-state synthesis (Phase 0-era + similar source-app pipeline corner cases). Sub-agent's Aretha Simons example is one instance; pre-Phase-3 audit query reveals the full 14 | Pre-Phase-3 audit query MUST return 0 before item 16 ships. Per-contact triage decided at item 16 kickoff time (synthesize personal state from legacy single-column fields IF contact still relevant; hard-delete genuinely abandoned rows IF no source-app reference + zero appointment_attendees rows + zero contact_programs rows + zero events) |

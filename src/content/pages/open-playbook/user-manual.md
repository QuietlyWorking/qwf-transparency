...
| **Documentation Center** | App components (React or SvelteKit) rendering a bundle generated from the manual by `render_<app>_docs.py` | `/docs` route in the app (public) | Derived from manual |
...
**Sync Cadence (reworded 2026-09-22 on TIG's "adopt"):** re-run the renderer and commit the bundle in the SAME session as any manual change; the app's proof suite runs `render_<app>_docs.py --check` and fails on drift, so /docs can never quietly disagree with the manual. The old "sync prompt every 5+ Lovable prompts" cadence survives only as the legacy path for apps that have no renderer yet (QWR today).
...
- **Model manual:** `002 Projects/_QWR Quietly Writing App/QWR-User-Manual.md` (v4.0.1); rendered reference implementation `002 Projects/_Quietly Quoting/QQT-User-Manual.md` (v1.0.2)
- **Renderer:** `005 Operations/Execution/render_qqt_docs.py` (manual -> `src/lib/docs/manual.json`; dry-run default, `--write`, `--check` exits non-zero on drift; reusable for any QWF manual via `--manual` / `--app`)
- **Proof:** `005 Operations/Execution/prove_qqt_docs.py` (deployed url, both engines; every H2 renders once, sidebar mirrors the manual, active section follows scroll, no auth, Help links resolve, no shell elements on the public page, drift gate red on a removed heading and on a hand-edited bundle)
...
| Writing a manual FROM the live app | Browser automation walk-throughs per role, evidence capture, "never document what you did not see" | ⭐⭐⭐ |
| Markdown-to-bundle renderer | Python text processing, deterministic output, structure-driven element mapping, drift gates | ⭐⭐⭐ |
| /docs Sync Prompts (legacy) | Lovable prompt engineering, React component design | ⭐⭐⭐ |
